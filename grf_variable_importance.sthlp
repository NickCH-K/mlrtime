...
{synopt:{opt print_only}} For especially long lists of predictors, or importance matrices for which all importances are zero, {cmd: rcall} may have some issues returning the results. If you have problems, you may try this option, which just prints out the R output and doesn't attempt to actually return any results to Stata.{p_end}
{synoptline}
{p2colreset}{...}
...
{pstd}
This is a postestimation function for {cmd:grf}, and can be used with any of the forests. See {cmd: rcall: help(variable_importance, package = 'grf')} or {browse "https://grf-labs.github.io/grf/"} for more information. Note that this will report variable importance using the {it: system} variable names, which likely don't match what you expect if you used factor variables or time-series operators in your list of predictors. Creating lag/lead or dummy variables ahead of time and naming them yourself is adviced if you plan to use {cmd: grf_variable_importance}.

{pstd}
This function creates an "importance value" of each predictor and how important it is in building the tree. It is a simple weighted sum of how many times a given predictor was split on at each depth in the forest. 

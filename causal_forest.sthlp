...
{synopt:{opt return(string)}} list of R commands, separated by semicolons ({cmd: ;}), each of which is of the format {cmd: newmacro = Rfunctioncreatingsomething}. Each of the {cmd: newvariable}s will be created in Stata as macros, accessible as {cmd: r(newmacro)} (as named). Names must be valid in R and Stata. These functions can refer to the causal forest object as {cmd: CF}, and the covariate, treatment, and outcome matrices as {cmd: X}, {cmd: W}, and {cmd: Y}, respectively. Use "@@" instead of "$" where necessary. For example, to get the average treatment effect and its standard errors, it would be {cmd: return(ate = average_treatment_effect(CF)[1]; ateSE = average_treatment_effect(CF)[2])} and then in Stata {cmd: display r(ate)} and {cmd: display r(ateSE)}. See {help rcall} documentation for the kinds of objects that will pass back to Stata properly. {p_end}
...
{synopt:{opt precedesCF}} list of R commands, separated by semicolons ({cmd: ;}), of whatever you'd like R to do just before running {cmd: grf::causal_forest}. These functions can refer to (and perhaps manipulate!) the covariate, treatment, and outcome matrices as {cmd: X}, {cmd: W}, and {cmd: Y}, respectively. Use "@@" instead of "$" where necessary.{p_end}
{synopt:{opt followsCF}} list of R commands, separated by semicolons ({cmd: ;}), of whatever you'd like R to do just after running {cmd: grf::causal_forest} but before generating any of the variables to return. These functions can refer to the causal forest object as {cmd: CF}, and the covariate, treatment, and outcome matrices as {cmd: X}, {cmd: W}, and {cmd: Y}, respectively. Use "@@" instead of "$" where necessary.{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
...
{pstd} 
One weakness of this command is that it does not automate working with training/holdout sets. Stata just isn't really made for that. However, this is not too difficult to do. Before running {cmd: causal_forest}, load your holdout data. Limit the dataset to just the covariates in {cmd: X}, and just the nonmissing observations. Send this to R as a matrix with {rcall: X.hold <- as.matrix(st.data())}. Then, load your training data and run {causal_forest} as normal, but instead of using {cmd: pred()} to create predictions, instead use {cmd: varreturn()} and do {cmd: pred <- as.matrix(predict(CF, X.hold)@@predictions)}. 

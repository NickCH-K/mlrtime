...
The default is "HC3", which is recommended in small samples and corresponds to 
the "shortcut formula" for the jackknife (see MacKinnon & White for more 
discussion, and Cameron & Miller for a review).
For large data sets with clusters, "HC0" or "HC1" are significantly faster to 
compute.{p_end}
{synoptline}
{p2colreset}{...}
...
{pstd}
{cmd:grf_test_calibration} is an interface between Stata and the 
"test_calibration" function in the R package {cmd: grf}, by Athey, Tibshirani, 
and Wager (2019).

{pstd}
This is a postestimation function for {cmd:grf}, and can be used with any of the
 forests.
See {cmd: rcall: help(test_calibration, package = 'grf')} or 
...
Computes the best linear fit of the target estimand using the forest prediction
 (on held-out data) as well as the mean forest prediction as the sole two
 regressors.
A coefficient of 1 for 'mean.forest.prediction' suggests that the mean forest
 prediction is correct, whereas a coefficient of 1 for 
 'differential.forest.prediction' additionally suggests that the forest has 
 captured heterogeneity in the underlying signal.
The p-value of the 'differential.forest.prediction' coefficient also acts as an
 omnibus test for the presence of heterogeneity: If the coefficient is 
 significantly greater than 0, then we can reject the null of no heterogeneity.
...
{phang} Cameron, A. C., and Miller, D. L. (2015). A Practitioner's Guide to 
...
{phang} MacKinnon, J. G., and White, H. (1985). Some 

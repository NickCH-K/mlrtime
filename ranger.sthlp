...
{synopt:{opt varreturn(string)}} see below.{p_end}
{synopt:{opt return(string)}} list of R commands, separated by semicolons ({cmd: ;}), each of which is of the format {cmd: newmac = Rfunctioncreatingsomething}. Each of the {cmd: newmac}s will be created in Stata in {cmd:r()}. Names must be valid in R and Stata. These functions can refer to the estimated {cmd: ranger} object as {cmd: RF}, and the data frame containing the data used in the {cmd: ranger} command as {cmd: rangerdata}. Use "@@" instead of "$" where necessary. For example, to get variable importance back as a matrix (after including an {cmd: importance} option in {cmd: opts}), it would be {cmd: return(importance = importance(RF))} and then in Stata {cmd: matrix list r(importance)}. See {help rcall} documentation for the kinds of objects that will pass back to Stata properly.{p_end}
...
{pstd}
On the {cmd: varreturn} option: This is a list of R commands, separated by semicolons ({cmd: ;}), each of which is of the format {cmd: newvariable = Rfunctioncreatingavariable}. Each of the {cmd: newvariable}s will be created in Stata as variables. Variable names must be valid in R and Stata. These functions can refer to the estimated {cmd: ranger} object as {cmd: RF}, and the data frame containing the data used in the {cmd: ranger} command as {cmd: rangerdata}. Use "@@" instead of "$" where necessary. Why is this option described down here? Becuase the SMCL was being weird.
